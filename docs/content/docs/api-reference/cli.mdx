...
| `langgraph`     | In-process LangGraph + Gateway provider | In-process LangGraph + your provider     |
...
The CLI applies the selected framework's final route/page files, dependencies, and deployment configuration over the base template. Both Vercel AI SDK variants are standard Next.js deployments with `streamText()`, `toUIMessageStreamResponse()`, and `vercelAIAdapter()`. The current LangGraph variants execute the graph in-process inside the Next.js `/api/chat` route. Both Eve variants wrap Next.js with `withEve()` and talk to Eve's native `/eve/v1/session*` protocol from the browser; a small client adapter maps Eve events to AG-UI for `agUIAdapter()`.

For Gateway-backed LangGraph and Vercel AI SDK variants, the framework owns orchestration and application tool execution. OpenUI Gateway is attached as the Responses/Chat Completions model provider and conversation store. Reports, presentations, web search, image search, and configured MCP tools remain Gateway-hosted tools. The Eve Gateway overlay uses Gateway as the Chat Completions provider and conversation store, without those hosted tools. Current LangGraph and Eve starters run with Next.js. Older LangGraph scaffolds or examples that proxy to a separate Agent Server still need that server deployed separately and a reachable `LANGGRAPH_API_URL`; inspect the generated files. See the [deployment guide](/docs/deploy).
...
For the full local-response → deploy → hosted-verification journey, follow [Deploy and share your app](/docs/deploy).
...
| Flag               | Description                                                                            |
| ------------------ | -------------------------------------------------------------------------------------- |
| `-y, --yes`        | Skip prompts; save missing file values only for explicitly selected preview/production |
...
Extra flags after `deploy` are forwarded to Vercel, which validates them. Pass the project directory positionally instead of `--cwd`, so validation and env loading use the same directory. `--skip-env` is OpenUI-specific.

Unlinked projects are linked first. OpenUI shows the project/team IDs, requested environment and local key names/sources before upload. Plain `deploy` leaves environment selection to Vercel; a new project's first deployment may be production. Use `--target preview` or `--prod` to request an environment, and check Vercel's reported environment afterward.

Allowlisted local values attach to this run's build and runtime; shell overrides `.env.local`, then `.env`. Saving file values for later is a separate choice, offered only for explicitly selected preview/production. Existing saved values are not overwritten. Declining still attaches local values. Default/custom environments use deployment-only local values; configure saved keys in Vercel. Explicit `--env`/`--build-env` flags take precedence and still work with `--skip-env`.

Build logs are quiet by default; use `--verbose` to stream them. `--no-wait` reports submission rather than a completed build. Verify a hosted response and intended sharing access after Ready; deployment does not configure application authentication.

```bash tab="pnpm" tab-group="pkg"
# Deploy from the project directory (Vercel chooses the environment)
...
immediate-start selection, and best-effort dev-command start and result events. For `deploy`, analytics include the target (currently `vercel`), requested and Vercel-reported environment, whether
...
Deployment analytics distinguish `--no-wait` submission from waiting-mode readiness and record
setup-stage outcomes. The post-create hint records printing, not reading. Website copy events
record which surface supplied the command, without changing the command itself. Existing telemetry opt-out
also disables hint experiment evaluation. No telemetry is added to generated apps.

See the [deployment telemetry contract](https://github.com/thesysdev/openui/blob/main/packages/openui-cli/TELEMETRY.md)
for event definitions, experiment attribution and dashboard recipes.
