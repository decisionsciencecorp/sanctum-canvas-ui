...
### Context summarization leaks into the stream

DeepAgents' built-in `SummarizationMiddleware` (deepagents ≤ 1.13.4) invokes the model without the
`nostream` tag, so the compaction summary is streamed like a normal assistant reply
([deepagentsjs#629](https://github.com/langchain-ai/deepagentsjs/issues/629)). The transformer
cannot distinguish it, and the plain-markdown summary breaks OpenUI Lang rendering once a thread
grows enough to trigger compaction. Until that is fixed upstream, override the middleware (DeepAgents
matches it by `name`) with LangChain's `summarizationMiddleware`, which tags its internal call:
