...
[Try the runnable Autofix example →](https://github.com/thesysdev/openui/tree/main/examples/miscellaneous/autofix)

The example uses AgentInterface to stream UI generated directly by OpenAI and automatically repair invalid output. Assistant replies show the final interface, original code, and repair diagnostics. Both API keys stay in a Next.js server route.
...
Point the OpenAI client at `/v1/autofix`. Your component library goes first, as a system or developer turn written by `generateSystemPrompt({ cloud: true, library })`; the generation goes last, as an assistant turn. Fenced or bare OpenUI Lang both work, and text outside the fence is ignored.

The API adds `fix_summary` to the chat completion, which the OpenAI types do not know about, so the example extends the response type.
...
This also applies when you render with OpenUI's built-in components: generate a spec for that library and pass it explicitly. `generateSystemPrompt({ cloud: true })` without a library is not sufficient, and the removed top-level `library` request field is rejected. Other system instructions still count toward the context limits.
...
- Send the recent context relevant to the repair. Requests exceeding these limits are rejected; trim older history before sending.
