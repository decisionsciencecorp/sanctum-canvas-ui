...
| `MarkDownRenderer` | **present** — `content/MarkDownRenderer.js`, registered, and in both catalogs |
| `Series` / `Slice` / `ScatterSeries` / `Point` | In both catalogs as chart-data contracts (`charts/dataLeaf.js`). Charts still read them from props. |
...
| `resources/libraries/{chat,dashboard}/library.v1.json` | **Full OpenUI vocabulary** (H21 deferral reopened and closed). Dashboard 82, chat 84. Generated by `tools/build-library-manifests.mjs`. |

### Catalog size (after H21 close)

| Library | Components in `library.v1.json` |
|---------|----------------------------------|
| chat | **84** — OpenUI chat library, root `Card` (no Stack, no Modal) |
| dashboard | **82** — OpenUI dashboard library, root `Stack` |

The six-name bootstrap (`Card`, `Stack`, `TextContent`, `Button`, `Input`, `BarChart`) was the execution deferral. It is not the catalog anymore.

Registry and prompt catalogs now name the same OpenUI vocabulary. The earlier six-name bootstrap was an execution deferral (H21), not the plan. It is closed.
