...
After an error-free, non-empty OpenUI renderer stream completes, a development-only card offers the deploy command and [deployment guide](https://www.openui.com/docs/deploy). It never runs a command, steals focus or opens Inspect. It is limited to localhost/loopback and stays out of production even when Inspect is explicitly enabled.

The card is shown once per browser origin (host + port) using the local `openui:deploy-hint:v1` storage key. Dismissal persists across reloads. This is not exact project identity: projects reusing a port share the choice; changing ports creates a separate origin. If storage is blocked, the hint stays hidden. Pass `deployHint={false}` to disable it.
...
| `deployHint`      | `true`           | Offer the one-time hint in development on localhost/loopback.            |
